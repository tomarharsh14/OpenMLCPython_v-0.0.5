...
def individual_data(indiv):
    ...
    x0 = np.array(g_data[0])  # Time divided by 10
    x1 = np.array(g_data[1])  # Temperature 
    x2 = np.array(g_data[2])  # Wind multiplied by 10^4
    x3 = np.array(g_data[3])  # Solar
    x4 = np.array(g_data[4])  # Humidity
    x5 = np.array(g_data[5])  # IsWeekend? (0/1)
    x6 = np.array(g_data[6])  # Day of the Week (1-7)
    x7 = np.array(g_data[7])  # IsNationalHoliday? (0/2)
    y = np.array(g_data[8])  # Whole Building Energy divided by 5
