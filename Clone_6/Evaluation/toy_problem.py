...
def individual_data(indiv):
    ...
    if isinstance(indiv.get_formal(), str):
        formal = indiv.get_formal().replace('S0','x0')  # Replacing S0 with x after obtaining the interpretable expression
    ...
    b = indiv.get_tree().calculate_expression([x0, x1, x2, x3, x4])
    # print b
    ...
    # ==============================================================================
    return x0, x1, x2, x3, x4, y, b


def cost(indiv):
    # x, y, y_with_noise, b = individual_data(indiv)
    x0, x1, x2, x3, x4, y, b = individual_data(indiv)
